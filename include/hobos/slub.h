...
#define MAX_NR_CACHE		(MAX_ORDER_KMEM - MIN_ORDER_KMEM + 1)
#define ORDER_OFFSET(order)	((order) - MIN_ORDER_KMEM)
...
/*
 * If we have more than 1 parent page, lets say if coming
 * from another processor or init is full, we go to *next
 */
struct kmem_cache {
	int order;
